import { type Atom, atom as createAtom } from 'jotai'
import {
...
function createPatchedStore(parentStore: Store, scope: Scope): ScopedStore {
  let storeState: any
  try {
    const v = getOriginalBuildingBlocks(parentStore)
    storeState = v.slice(0, 6) as BuildingBlocks
  } catch (e) {
    console.log(parentStore, (parentStore as any)[ORIGINAL_BUILDING_BLOCKS])
    throw e
  }
  const internalStoreHooks: StoreHooks = {}
  storeState[6] = internalStoreHooks
  storeState[9] = (atom: AnyAtom) => {
    atom.unstable_onInit?.(scopedStore)
  }
  const prototypeStore = INTERNAL_buildStore(...storeState)
  const buildingBlocks = INTERNAL_getBuildingBlocks(prototypeStore)
  ...
  const BUILDING_BLOCKS = Object.getOwnPropertySymbols(prototypeStore)[0]

  const scopedStore: ScopedStore = {
    get(atom, ...args) {
      const [scopedAtom] = scope.getAtom(atom)
      return prototypeStore.get(scopedAtom, ...args)
    },
    set(atom, ...args) {
      ...
      try {
        return prototypeStore.set(scopedAtom, ...args)
      } finally {
        restore?.()
      }
    },
    sub(atom, ...args) {
      const [scopedAtom] = scope.getAtom(atom)
      return prototypeStore.sub(scopedAtom, ...args)
    },
    [SCOPE]: scope,
    [BUILDING_BLOCKS]: scopedBuildingBlock,
